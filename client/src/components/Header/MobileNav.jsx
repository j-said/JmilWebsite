...
export default function MobileNav({ isOpen, onClose, isDark, toggleTheme }) {
    return (
        <>
            {/* Overlay */}
            <div
                className={`fixed inset-0 bg-black bg-opacity-50 z-40 md:hidden transition-opacity duration-300 ${isOpen ? 'opacity-50' : 'opacity-0 pointer-events-none'
                    }`}
                onClick={onClose}
            />
            ...
            {/* Sidebar */}
            <div
                className={`fixed top-0 right-0 h-full w-80 bg-[var(--background)] shadow-2xl z-50 transform transition-transform duration-300 ease-in-out md:hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}
            >
                {/* Close button */}
                ...
                <nav className="p-6">
                    ...
                    {/* Theme toggle */}
                    <div className="mt-8 pt-6 border-t border-[var(--muted)]">
