...
export default function ProductGallery({ product }) {
    ...
    const images = [
        product.imageUrl,
        ...(product.images || [])
    ].filter(Boolean); // Remove potential null/undefined
    ...
    return (
        <div className="space-y-4 theme-transition-colors">
            {/* Main Image Frame */}
            <div
                className="relative rounded-2xl overflow-hidden border border-[var(--muted)] shadow-lg group bg-[var(--background)] aspect-[4/3] cursor-zoom-in theme-transition-colors"
                onClick={openModal}
            >
                <img
                        ...
                        <button
                            ...
                            className={`
                                ...
                                ${selectedImageIndex === idx
                                    ...
                                    : 'border-[var(--muted)] opacity-70 hover:opacity-100 hover:border-brand-orange'
                                }
                            `}
                        >
            ...
            {/* Lightbox Modal */}
            {isModalOpen && (
                <div className="fixed inset-0 z-[60] bg-black/90 flex items-center justify-center p-4 animate-fadeIn" onClick={closeModal}>
                    <button
                    ...
                    <div className="relative max-w-5xl w-full max-h-[90vh] flex items-center justify-center">
                        {images.length > 1 && (
                            <button onClick={handlePrev} className="absolute -left-12 text-white p-4 hover:text-brand-orange transition-colors">
                        ...
                        {images.length > 1 && (
                            <button onClick={handleNext} className="absolute -right-12 text-white p-4 hover:text-brand-orange transition-colors">
