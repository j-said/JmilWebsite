...
export default function ProductCard({ product }) {
  ...
  const imageUrl =
    product.imageUrl ||
    `https://placehold.co/400x300/1a1a1a/FFA500?text=${encodeURIComponent(
      product.name
    )}`;
  ...
  return (
    <div
      className={`group relative border border-[--muted] rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col h-full ${
    ...
    >
      <Link to={`/shop/${product.id}`} className="block flex-1">
        <div className="relative overflow-hidden rounded-t-xl">
            ...
            className={`absolute top-3 right-3 p-2 rounded-full transition-all duration-300 shadow-md z-10
                            ${
                              isWishlisted
                                ...
                                : "bg-[--background]/80 text-[--foreground] hover:bg-brand-yellow hover:text-brand-black"
                            }`}
            aria-label={
          ...
          {/* Badges rendering code... (без змін) */}
          {product.onSale && (
        ...
        {/* Product Info code... (без змін) */}
        <div className="p-4">
          <h3 className="font-bold text-[--foreground] mb-2 line-clamp-2 theme-transition-colors group-hover:text-brand-orange">
            {product.name}
          </h3>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {product.rating && (
              <div className="flex items-center space-x-1 bg-[--muted]/30 px-2 py-1 rounded-lg">
